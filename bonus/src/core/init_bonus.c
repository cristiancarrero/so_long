...
int load_enemy_images(t_game *game)
{
	...
	{
		if (game->enemies[i].type == 1)  // Enemigo tipo X (horizontal)
		{
			// Cargar los tres frames de animación
			game->enemies[i].sprites[0] = mlx_xpm_file_to_image(game->mlx,
				SPRITE_ENEMY1, &width, &height);
			game->enemies[i].sprites[1] = mlx_xpm_file_to_image(game->mlx,
				SPRITE_ENEMY2, &width, &height);
			game->enemies[i].sprites[2] = mlx_xpm_file_to_image(game->mlx,
				SPRITE_ENEMY3, &width, &height);

			if (!game->enemies[i].sprites[0] || !game->enemies[i].sprites[1] || 
				!game->enemies[i].sprites[2])
			{
				ft_putendl_fd("Error: No se pudieron cargar los sprites del enemigo X", 2);
				return (0);
			}
			game->enemies[i].current = game->enemies[i].sprites[0];
		}
		else  // Enemigo tipo N (vertical)
		{
			// Cargar los tres frames de animación para el enemigo tipo 2 (hongo)
			game->enemies[i].sprites[0] = mlx_xpm_file_to_image(game->mlx,
				SPRITE_ENEMY2_1, &width, &height);
			game->enemies[i].sprites[1] = mlx_xpm_file_to_image(game->mlx,
				SPRITE_ENEMY2_2, &width, &height);
			game->enemies[i].sprites[2] = mlx_xpm_file_to_image(game->mlx,
				SPRITE_ENEMY2_3, &width, &height);

			if (!game->enemies[i].sprites[0] || !game->enemies[i].sprites[1] || 
				!game->enemies[i].sprites[2])
			{
				ft_putendl_fd("Error: No se pudo cargar el sprite del enemigo N", 2);
				return (0);
			}
			game->enemies[i].current = game->enemies[i].sprites[0];
		}
