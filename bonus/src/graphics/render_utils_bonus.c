...
static void	put_pixel_with_flip(t_game *game, t_copy_params p, int i, int j)
{
	...
	if (color != 0xFF000000)  // No copiar píxeles transparentes
	{
		if ((p.is_player && game->is_facing_left) || 
			(!p.is_player && p.direction == 0))
		{
			x = p.x * TILE_SIZE + (TILE_SIZE - 1 - j);  // Voltear horizontalmente
		}
		else
		{
			x = p.x * TILE_SIZE + j;
		...
		if (x >= 0 && x < game->buffer.width && y >= 0 && y < game->buffer.height)
		{
