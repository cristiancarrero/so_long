...
int	validate_map(t_game *game)
{
	...
	for (int y = 0; y < game->map_height; y++)
	{
		for (int x = 0; x < game->map_width; x++)
		{
			if (game->map[y][x] == 'X')  // Enemigo tipo 1 (movimiento horizontal)
			{
				game->enemies[enemy_index].pos.x = x;
				game->enemies[enemy_index].pos.y = y;
				game->enemies[enemy_index].type = 1;
				game->enemies[enemy_index].direction = 0;
				game->enemies[enemy_index].active = 1;
				// Cargar los sprites de animación
				game->enemies[enemy_index].sprites[0] = mlx_xpm_file_to_image(game->mlx, SPRITE_ENEMY1, &width, &height);
				game->enemies[enemy_index].sprites[1] = mlx_xpm_file_to_image(game->mlx, SPRITE_ENEMY2, &width, &height);
				game->enemies[enemy_index].sprites[2] = mlx_xpm_file_to_image(game->mlx, SPRITE_ENEMY3, &width, &height);
				game->enemies[enemy_index].current = game->enemies[enemy_index].sprites[0];
				
				game->enemies[enemy_index].frame = 0;
				enemy_index++;
			}
			else if (game->map[y][x] == 'N')  // Enemigo tipo 2 (movimiento vertical)
			{
				game->enemies[enemy_index].pos.x = x;
				game->enemies[enemy_index].pos.y = y;
				...
				game->enemies[enemy_index].direction = 0;
				game->enemies[enemy_index].active = 1;
				// Cargar sprite estático para enemigo vertical
				game->enemies[enemy_index].current = mlx_xpm_file_to_image(game->mlx, SPRITE_ENEMY1, &width, &height);
				// Decidir dirección inicial basada en obstáculos
